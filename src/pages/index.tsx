  ...
  return (
    <div className='min-h-screen flex justify-center items-center bg-black px-4'>
      <div className='flex flex-col md:flex-row justify-center items-center gap-10 max-w-5xl w-full mx-auto'>
        ...
        <div className='grid grid-cols-1 gap-4 w-full md:w-1/2 bg-blue-600'>
          {Array(4)
            ...
            .map((_, index) => (
              <div
                key={index}
                className='w-2/5 h-[15vh] border-3 border-white rounded-xl shadow-md flex justify-center items-center bg-white'
              >
                {photos[index] ? (
                  <img
                    src={photos[index]}
        ...
        {countTime !== null && (
          <span className='text-black text-6xl font-bold '>{countTime}</span>
        )}
      </div>
    </div>
  );
